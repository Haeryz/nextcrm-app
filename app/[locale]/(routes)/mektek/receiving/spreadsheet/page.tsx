...
export default async function MektekReceivingSpreadsheetPage({
  params,
  searchParams,
  ...
  if (!canManageMektekLogistics(session?.user)) {
    return (
      <Container
        ...
        description="Tracking Purchase Order ke supplier dan barang masuk"
      >
        <Card>
  ...
  const pageHref = (targetPage: number) => {
    ...
    return `/${locale}/mektek/receiving/spreadsheet?${paramsForPage.toString()}`;
  };
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <CardContent className="p-4">
            <form
              action={`/${locale}/mektek/receiving/spreadsheet`}
              className="grid gap-3 md:grid-cols-[minmax(240px,1fr)_180px_auto_auto]"
            >
              <Input
                name="q"
                type="search"
                placeholder="Cari PO, supplier, User/PT, project, atau item..."
                defaultValue={query}
              ...
              {(query || status) && (
                <Button asChild type="button" variant="ghost">
                  <Link href={`/${locale}/mektek/receiving/spreadsheet`}>Reset Filter</Link>
                </Button>
              )}
            </form>
          </CardContent>
